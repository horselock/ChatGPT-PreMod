...
# Changelog
- 2.3.0 - Fixed PreMod not loading at all while logged in: ChatGPT's Content Security Policy now blocks the blob: script PreMod injected. The page code is now injected via GM_addElement where available, falling back to reusing the page's script nonce (e.g. iOS Userscripts). Also: banner settings moved out of the script and into storage, so they survive updates. Toggle all banners, or just the "PreMod Active" startup banner, from your userscript manager's menu. Replaces the SHOW_BANNERS constant. Startup banner toggle idea from sn-o-w (#1).
